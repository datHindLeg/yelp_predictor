...
def main():
    ...
    data_retained, total_reviews, total_data_pts, df_train_name, df_train_text, df_train_period_rating, df_train_exo, df_train_score, df_validation_name, df_validation_text, df_validation_period_rating, df_validation_exo, df_validation_score, df_train, df_validation = load_data(df_training,df_test, df_rejects)
    ...
    m,b = np.polyfit(df_training['period_rating'], df_training['health_score'],1)
    ...
    plt.plot(df_training['period_rating'], df_training['health_score'], '.')
    plt.plot(df_training['period_rating'], m*df_training['period_rating'] + b, '-')
    plt.show()

    # regression to determine health score based on number of yelp reviews
    # coefficient that is printed if the amount health score is expected to change based on a 1 unit change increase in number reviews (number_reviews)
    values_2,residuals_2,rank_2,singular_values_2,rcond_2 = np.polyfit(df_training['number_reviews'], df_training['health_score'],1, full=True)
    n,a = np.polyfit(df_training['number_reviews'], df_training['health_score'],1)
    print "\nFor every 1 point increase in in number of yelp reviews, health score changes by " + str(values_2[0]) + " points."
    print "Sum of squared residuals (large = bad) : " + str(residuals_2[0])
    plt.plot(df_training['number_reviews'], df_training['health_score'], '.')
    plt.plot(df_training['number_reviews'], n*df_training['number_reviews'] + a, '-')
    plt.show()
    ...
    # heavy lifting regression on variables unrelated to review text
    model_train_count_exo, model_validation_count_exo, feature_names_exo = get_features_exo(df_train_exo, df_validation_exo)
    values_exo = ridge_regression(model_train_count_exo,df_train_exo['health_score'],model_validation_count_exo,df_validation_exo['health_score'], feature_names_exo)
...
def load_data(train, test, rejects):
    ...
    df_train_price_category = df_train['price_category']
    df_train_period_rating = df_train['period_rating']
    df_train_score = df_train['health_score']
    df_train_exo = df_train[['period_rating','category','price_category','health_score']]

    df_validation_name = df_validation['name']
    df_validation_text = df_validation['review_text']
    df_validation_category = df_validation['category']
    df_validation_price_category = df_train['price_category']
    df_validation_period_rating = df_validation['period_rating']
    df_validation_score = df_validation['health_score']
    df_validation_exo = df_train[['period_rating','category','price_category','health_score']]

    return data_retained, total_reviews, total_data_pts, df_train_name, df_train_text, df_train_period_rating, df_train_exo, df_train_score, df_validation_name, df_validation_text, df_validation_period_rating, df_validation_exo, df_validation_score, df_train, df_validation
...
def get_features_exo(df_train_exo,df_validation_exo):
    train_master = []
    validation_master = []
    vec = DictVectorizer()

    # for each row add features to dict, then append to master

    f_train = lambda row: train_master.append( {'period_rating': row['period_rating'], 'category': row['category'], 'price_category': row['price_category'] } )
    f_valid = lambda row: validation_master.append( {'period_rating': row['period_rating'], 'category': row['category'], 'price_category': row['price_category'] } )

    df_train_exo.apply(f_train,axis=1 )
    X_train = vec.fit_transform(train_master).toarray()

    df_validation_exo.apply(f_valid,axis=1 )
    X_validation = vec.transform(validation_master).toarray()

    return X_train, X_validation, vec.get_feature_names()
...
def get_features(train_text,train_targets,validation_text,validation_targets):
    ...
    adjs = set(df_adj.tolist())
    nouns = set(df_nouns.tolist())
    ...
    fr = open("/home/datascience/FINAL_PROJECT/yelp_predictor/tfidf_weights.npy","w")
    np.save(fr, pairings)
    fr.close()

    # combine Tfidf with DictVectorizer
...
def ridge_regression(train_model, train_targets, validation_model, validation_targets, features):
    regr = linear_model.Ridge(alpha=.001,normalize=True)
    regr.fit(train_model, train_targets)
    feature_to_coef = {}
    for pairing in zip(features,regr.coef_):
        feature_to_coef[pairing[0]] = pairing[1]

    fe = open("/home/datascience/FINAL_PROJECT/yelp_predictor/tfidf_weights.txt","w")
    for pair in feature_to_coef.iteritems():
        fe.write(pair[0] + ":" + str(pair[1]) + "\n")
    fe.close()
    print '\nmax coef value: ' + str(max(feature_to_coef.itervalues()))
    print 'min coef value: ' + str(min(feature_to_coef.itervalues()))
    print '\nFeatures with highest regression coefficient values (positive in order):\n ' + str(sorted(feature_to_coef,key=feature_to_coef.get,reverse=True)[:150]) + '\n'
    ...
    #plt.scatter(validation_model, validation_targets,  color='black')
    #reshaped = validation_model.reshape(97029,-1).flatten()
    reshaped = validation_model.toarray().flatten()
    plt.plot(reshaped, regr.predict(validation_model), color='blue',
                     linewidth=3)
...
def plot_regression(v_model, v_targets, regr_model):
    plt.scatter(v_model, v_targets,  color='black')
    plt.plot(v_model, regr_model.predict(v_model), color='blue',
                     linewidth=3)
...
def plot_basic_regr(coef, inter):
    r_x, r_y = zip(*((i, i*coef + inter) for i in range(5)))
